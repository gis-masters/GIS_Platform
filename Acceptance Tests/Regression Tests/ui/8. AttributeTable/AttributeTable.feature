...
Scenario: фильтрация строковых объектов
    Given открыта таблица атрибутов, открыты фильтры
    When пользователь вводит значения в одном из строковых полей
    Examples:
        | dd |
        | DD |
    Then в таблице отображаются обьекты соответственно фильтру вне зависимости от регистра
...
Scenario: Поле типа LOOKUP не отображается
    ...
    When пользователь открывает таблтцу атрибутов
