Feature: Восстановление пароля. API инициализации.
  ...
  Scenario: При попытке восстановления пароля, отправляя неверный токен, возвращается ответ 400
    Given Отправляется POST запрос на эндпоинт request-password-reset, с телом в котором содержится поле email и originHost
  ...
  Scenario Outline: Попытка восстановления пароля с невалидными значениями (<reason>)
    Given Пользователь делает запрос на восстановление пароля с невалидными данными "<email>" "<originHost>"
    Then Сервер отвечает со статус-кодом 400
    Examples:
      | email    | originHost | reason                                     |
      | EMAIL_0  | STRING_50  | email: Пустое значение не допускается      |
      | EMAIL_15 | STRING_0   | originHost: Пустое значение не допускается |

  Scenario: Проверить актуальность токена для восстановления пароля можно отправив GET запрос на эндпоинт
  /password-reset, передав проверяемый токен в параметре: "token"
