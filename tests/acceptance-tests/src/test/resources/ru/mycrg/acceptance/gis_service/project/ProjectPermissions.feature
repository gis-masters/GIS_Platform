Feature: Доступность и управление проектами согласно правам
  ...
  Scenario: Выборка проходной папки проекта корректно отрабатывает для пользователя
    *    я назначил роль "CONTRIBUTOR" для пользователя "fiz5" на "Проект1.1"
    *    я авторизован как "fiz5"
    When я делаю запрос на папку проектов "Папка1"
    Then атрибут "name" имеет значение "Папка1"
