...
Feature: Тесты осуществления услуги "Выдача разрешения на строительство объекта капитального строительства"
  ...
  Background:
    ...
    *     Текущему пользователю установлена роль "CONTRIBUTOR", для библиотеки: "dl_data_inbox_data"
    *     Текущему пользователю установлена роль "CONTRIBUTOR", для каталога 40 в библиотеке "dl_data_inbox_data"
...
#      | gpzu_smev_rostelekom | ГПЗУ дубликат | CANCELED    | 8                   | Дубликат ГПЗУ Заявление отменено         |
  ...
  Scenario Outline: Когда пользователь ставит Промежуточный статус: 5 вложение успешно отправляется
    ...
    And   Текущему пользователю установлена роль "CONTRIBUTOR", для библиотеки: "dl_data_section13"
    *     Текущему пользователю установлена роль "CONTRIBUTOR", для библиотеки: "dl_data_section_delivery_data"
