...
Feature: Доступность и управление задачами согласно иерархии пользователей
  ...
  Scenario: Владельцу организации доступны все задачи (не зависит от иерархии)
    Given я авторизован как "orgOwner"
  ...
  Scenario Outline: Редактироваь возможно только свои задачи и задачи непосредственных подчиненных. <reason>
    ...
    Examples:
      ...
      | fiz4     | fiz4     | fiz4     | 204      | I'll be back!   | Своя задача                                |
