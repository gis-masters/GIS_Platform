Feature: Публикация GML
  ...
  Background:
    ...
    *     Существует проект "Гончие псы"

  Scenario: При публикации GML создаётся новый набор данных, все слои размещаются в этом наборе данных
    *     Существует файл "correct.gml"
    *     к текущему документу прикреплён текущий файл
  ...
  Scenario: При публикации GML данные строки обрезаются если их длинна превышает заданное схемой ограничение

  Description:  В тестовом файле address единственное поле с ограничением длинны в схеме.
  Warnings:     Тест изменяет стандартную схему 10 приказа recreation_point

    *     Существует файл "long_string.gml"
    *     к текущему документу прикреплён текущий файл
    *     Существует схема "Схема recreation_point"
    When  пользователь публикует текущий GML
    Then  процесс завершается успешно
    And   в результате публикации был создан набор данных
    *     в результате публикации была создана таблица
    *     в результате публикации была создана запись с id: 1
    *     атрибут "[0].properties.address" имеет длину 255 символов
  ...
  Scenario: Импорт GML успешно создаёт объекты любой валидной геометрии
    ...
    *     к текущему документу прикреплён текущий файл
    When  пользователь публикует текущий GML
    Then  процесс завершается успешно
    *     количество таблиц в созданном наборе данных равно: 10
    *     данные внутри каждой из таблиц соответствует ожиданиям
      | vectorTable                | globalid     | shape                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
