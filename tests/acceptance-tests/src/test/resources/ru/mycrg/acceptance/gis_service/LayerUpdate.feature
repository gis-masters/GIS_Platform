Feature: Обновление слоя проектов
  ...
  Scenario Outline: Обновление слоя проекта администратором организации
    ...
    Examples:
      | title    | dataset  | internalName | styleName | type   | schemaId | dataStoreName | nativeCRS  | dataSourceUri | newTitle | newDataset | enabled | position | transparency | minZoom | maxZoom | newNativeCrs |
  ...
  Scenario Outline: Обновление слоя проекта пользователем организации
    ...
    Examples:
      | userName  | userSurname | userEmail | userPassword | title    | dataset  | internalName | styleName | type   | schemaId | dataStoreName | nativeCRS  | dataSourceUri | newTitle | newDataset | enabled | position | transparency | minZoom | maxZoom | newNativeCrs |
