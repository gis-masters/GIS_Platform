Feature: Проверка API установки прав на наборы данных
  ...
  Scenario: Создание дублирующегося правила должно откланяться сервером с 409 кодом ошибки
    Given Существуют заданное кол-во наборов: 1
  ...
  Scenario Outline: Создание правил c НЕ валидными данными (<reason>) должно откланяться сервером с 400 кодом ошибки
    Given Существуют заданное кол-во наборов: 1
    When Отправляется запрос на создание правила для текущего набора данных "<role>" "<principalId>" "<principalType>"
  ...
  Scenario: Администратор организации должен видеть все разрешения
    ...
    And Количество правил соответствует ожидаемому: "11"

  Scenario: Пользователь не являющийся владельцем и не имеющий роли OWNER, имеет доступ только к своим собственным
  правилам
    Given Существуют заданное кол-во наборов: 1
    Given Текущему набору задаётся некое кол-во правил: 9
    Given Существует пользователь
  ...
  Scenario: Пользователи с ролью OWNER имеют доступ ко всем правилам
    ...
    When Авторизируемся пользователем
    When Пользователь запрашивает правила для текущего набора
    And Количество правил соответствует ожидаемому: "8"

  Scenario Outline: Владелец набора имеют доступ ко всем правилам
    Given Существует пользователь
      | STRING_5 | STRING_10 | EMAIL_7 | testtestQ1 |
    Given Авторизируемся пользователем
    Given Отправляется запрос на создание набора "<title>" "<description>"
