Feature: Права на библиотеки
  ...
  Scenario: Если пользователь находится в нескольких группах, имеющих разные права доступа к библиотеке, то для него выбирается лучшая роль
    ...
    Given Существует пользовательская группа "VIEWERS", "имеет права на чтение"
    Given Администратор добавляет пользователя в пользовательскую группу
    Given Владелец организации устанавливает роль "VIEWER" для текущей группы, для библиотеки: "dl_default"
    Given Существует пользовательская группа "CONTRIBUTORS", "имеет права на запись"
    Given Администратор добавляет пользователя в пользовательскую группу
    Given Владелец организации устанавливает роль "CONTRIBUTOR" для текущей группы, для библиотеки: "dl_default"
    Given Авторизируемся пользователем
