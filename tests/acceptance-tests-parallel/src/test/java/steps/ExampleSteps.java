package steps;

import io.cucumber.datatable.DataTable;
import io.cucumber.docstring.DocString;
import io.cucumber.java.en.And;
import io.cucumber.java.en.Given;
import io.cucumber.java.en.Then;
import io.cucumber.java.en.When;
import io.cucumber.java.no.Gitt;
import io.cucumber.java.no.Og;
import io.cucumber.java.ro.Atunci;
import io.cucumber.java.ro.Cand;
import io.cucumber.java.ro.Datefiind;

import static java.lang.Thread.sleep;

@SuppressWarnings("unused")
public class ExampleSteps {
    @Given("^this is a given step$")
    public void thisIsAGivenStep() throws InterruptedException {
        sleep(1000);
    }

    @When("^I do something$")
    public void iDoSomething() throws InterruptedException {
        sleep(1000);
    }

    @Then("^I expect a result$")
    public void iExpectAResult() throws InterruptedException {
        sleep(1000);
    }

    @And("^I expect a second result$")
    public void iExpectASecondResult() throws InterruptedException {
        sleep(1000);
    }

    @Given("^this is background given step$")
    public void thisIsBackgroundGivenStep() throws InterruptedException {
        sleep(1000);
    }

    @And("^this is another background step$")
    public void thisIsAnotherBackgroundStep() throws InterruptedException {
        sleep(1000);
    }

    @When("^I do something with data$")
    public void iDoSomethingWithData(DataTable dataTable) throws InterruptedException {
        sleep(1000);
    }

    @Given("^this is a background step with data$")
    public void thisIsABackgroundStepWithData(DataTable dataTable) throws InterruptedException {
        sleep(1000);
    }

    @And("^this is a background step$")
    public void thisIsABackgroundStep() throws InterruptedException {
        sleep(1000);
    }

    @Given("^this is a given step with a docstring$")
    public void thisIsAGivenStepWithADocstring(DocString docString) throws InterruptedException {
        sleep(1000);
    }

    @Then("^I do something with a docstring$")
    public void iDoSomethingWithADocstring(DocString docString) throws InterruptedException {
        sleep(1000);
    }

    @Gitt("^for å unngå at firmaet går konkurs$")
    public void norwegian1() throws InterruptedException {
        sleep(1000);
    }

    @Og("^må regnskapsførerere bruke en regnemaskin for å legge sammen tall$")
    public void norwegian2() throws InterruptedException {
        sleep(1000);
    }

    @Datefiind("^Bună dimineața$")
    public void romanian1() throws InterruptedException {
        sleep(1000);
    }

    @Cand("^Scuzați-mă!$")
    public void romanian2() throws InterruptedException {
        sleep(1000);
    }

    @Atunci("^Vă doresc o zi plăcută!$")
    public void romanian3() throws InterruptedException {
        sleep(1000);
    }

    @Cand("^Scuzați-mă <test>$")
    public void romanianDataTable(DataTable dataTable) throws InterruptedException {
        sleep(1000);
    }

    @Cand("^Scuzați-mă (\\d)$")
    public void romanianValue(int value) throws InterruptedException {
        sleep(1000);
    }

    @Given("^I am on a page with text '(.*)'$")
    public void iAmOnAPageWithTextText(String text) throws InterruptedException {
        sleep(1000);
    }
}