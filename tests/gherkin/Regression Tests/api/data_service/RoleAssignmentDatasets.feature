...
#    Then получаем ответ 403
  ...
  Scenario: Добавление роли
    ...
    When Попытка добавить любые ограниченичя: "/api/data/datasets/workspace_1/tables/some_table/roleAssignment"
    Then приводит к 404 т.к. ресурса нет
    And
    Given Существует схема "workspace_1" в ней таблица "electricpowerstation"
    When Выполнить POST запрос на эндпоинт: "/api/data/datasets/workspace_1/tables/electricpowerstation/roleAssignment"
          с телом:
          {
            "role": "VIEWER",
            "principalType": "group",
    ...
    When Выполняя повторный запрос на создание тех же ограничений.
          POST запрос на эндпоинт: "/api/data/datasets/workspace_1/tables/electricpowerstation/roleAssignment"
          с телом:
          {
            "role": "VIEWER",
            "principalType": "group",
  ...
  Scenario: Получение пермишенов
    ...
    When Выполнить GET запрос на эндпоинт: "/api/data/datasets/workspace_1/tables/electricpowerstation/roleAssignment"
  ...
  Scenario: Удаление всех пермишенов
    ...
    When Выполнить DELETE запрос на эндпоинт: "/api/data/datasets/workspace_1/tables/electricpowerstation/roleAssignment"
  ...
  Scenario: Удаление пермишена с ид 1
    Given Для схемы "workspace_1" таблицы "electricpowerstation" заданы пермишены
    When Выполнить DELETE запрос на эндпоинт: "/api/data/datasets/workspace_1/tables/electricpowerstation/roleAssignment/1"
    Then Сервер отвечает 204
