...
class EditFeatureHistoryStore {
  ...
  /**
   * Красиво распечатывает текущую историю в консоль
   */
  printHistory(): void {
    services.logger.trace('📚 История редактирования геометрии:');
    services.logger.trace('═'.repeat(50));

    if (this.history.length === 0) {
      services.logger.trace('📝 История пуста');
    ...
    this.history.forEach((entry, index) => {
      ...
      services.logger.trace(`${marker} [${index}] ${entry.description}`);
      services.logger.trace(`    ID: ${entry.id} | Время: ${timestamp}`);
      services.logger.trace(`    Тип геометрии: ${entry.geometry.type}`);
      services.logger.trace(`    Координаты: ${JSON.stringify(entry.geometry.coordinates)}`);

      if (index < this.history.length - 1) {
        services.logger.trace('    ' + '─'.repeat(40));
      }
    });
