Feature: xTable Cell Content Type String
  ...
  Scenario: Внешний вид ячейки таблицы с очень длинной строкой
    Given я на странице "xtable-cellcontent--text-overflow" библиотеки блоков
    Then блок XTableCellContentTypeString вариант "hide-text" она выглядит как положено

  Scenario: Внешний вид ячейки таблицы с очень длинной строкой в развёрнутом виде
    Given я на странице "xtable-cellcontent--text-overflow" библиотеки блоков
    When я нажимаю кнопку `Показать всё`
    Then блок XTableCellContentTypeString вариант "show-text" она выглядит как положено
