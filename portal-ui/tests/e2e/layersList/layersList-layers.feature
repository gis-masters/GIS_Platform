Feature: Список слоёв: слои
  ...
  Scenario: Пользователь может разместить .tif файл в проекте с помощью кнопки "Разместить в проекте"
    ...
    *     существует библиотека "документы с простым контент типом" документов по заготовленной схеме с включенным версионированием
 ...
 # работает через раз, будет починено в #778
  @skip()
  Scenario Outline: Пользователь не видит растровый слой если его источника данных нет в документе
    Given у пользователя "Гарри" есть право на "Запись" на библиотеку документов "Тестовая библиотека"
    *     все документы в библиотеке "Тестовая библиотека" удалены
    *     в библиотеке документов "Тестовая библиотека" существует минимум 1 документов, доступных пользователю "Гарри"
    *     загружен тестовый файл "<файл>"
