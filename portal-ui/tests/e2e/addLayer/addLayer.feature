Feature: Добавление слоя на карте
  ...
  Scenario Outline: Пользователь может создать слой из файла "<тип>" на странице карты
    ...
    *     существует библиотека документов "документы с простым контент типом в новой библиотеке" по заготовленной схеме без версионирования
    ...
    *     в диалоговом окне `Добавить слой` я выбираю тип слоя 'Файловый'
    ...
    *     в explorer я захожу внутрь элемента с названием "документы с простым контент типом в новой библиотеке"
    *     в explorer я захожу внутрь созданного документа
