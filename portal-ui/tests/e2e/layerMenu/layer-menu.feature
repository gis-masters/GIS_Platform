Feature: Меню слоя
  ...
  Scenario: Пользователь может открыть атрибутивную таблицу для векторных слоев
    ...
    *     пользователем "Гарри" внутри набора данных "набор админ деление" создана таблица "админ деление с представлениями" по схеме "Схема с представлениями"
    *     пользователем "Гарри" создан проект "Проект Ялты"
    *     администратором создан слой с параметрами:
  ...
  Scenario: При открытии формы добавления объекта отсутствуют уведомления об ошибках
    Given существует заготовленная схема "Схема в режиме редактирования"
    *     существует набор данных, созданный пользователем "Гарри"
    *     внутри созданного набора данных существует таблица по схеме "Схема в режиме редактирования" созданная пользователем "Гарри"
    ...
    *     я нахожусь на странице карты проекта
    ...
    Then  открывается панель редактирования объекта
  ...
  Scenario: При удалении слоя выделение с объектов этого слоя снимается, атрибутивная таблица закрывается
    ...
    *     существует набор данных, созданный пользователем "Гарри"
    *     внутри созданного набора данных существует таблица по схеме "Схема для тестирования сортировки" созданная пользователем "Гарри"
    *     существует проект, созданный пользователем "Гарри"
