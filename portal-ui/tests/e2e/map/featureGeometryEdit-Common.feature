Feature: Редактирование геометрии объекта

  Background:
    Given все проекты удалены
    *     все наборы данных удалены

  Scenario: Активация режима редактирования геометрии объекта кнопкой "Редактировать геометрию" включает режим снеппинга (с красными точками и активной кисточкой)
    ...
    *     в форме просмотра объекта, я перехожу на вкладку просмотра геометрии
    # кнопка `Редактировать геометрию` после нажатия отображается как активная
    *     в вкладке просмотра геометрии я нажимаю кнопку `Редактировать геометрию`
    # переводим курсор, чтобы не попал в кадр тултип
  ...
  Scenario: Изменение геометрии объекта, при котором происходит выход за текущий bbox слоя +200 км, подсвечивает, возможно некорректное значение
    ...
    *     я на странице карты проекта, открыт объект с id 1
    When  в форме просмотра объекта, я перехожу на вкладку просмотра геометрии
    *     у координаты с номером 1 в поле `X` устанавливаю значение 499000
  ...
  Scenario: Исправление и успешное сохранение невалидной геометрии при создании нового объекта
    ...
    *     у координаты с номером 1 устанавливаю значения X: 10 и Y: 10
    *     у координаты с номером 2 устанавливаю значения X: 20 и Y: 20
    *     у координаты с номером 3 устанавливаю значения X: 10 и Y: 20
    *     в геометрии первого контура я добавляю новый узел
    *     у координаты с номером 4 устанавливаю значения X: 20 и Y: 10
    *     в форме редактирования объекта я нажимаю кнопку `Сохранить`
    *     я дожидаюсь исчезновения индикатора загрузки в форме редактирования объекта
  ...
  Scenario: Геометрия фичи и ее отображение на карте восстанавливается к первоначальным значениям при выходе из
  объекта без сохранения внесенных в геометрию изменений
    ...
    *     существует проект "Проект зверей охраняющих дикую природу", созданный пользователем "Гарри"
    *     в текущий проект подключена пустая подложка
    *     в созданном проекте создан слой "просто слой" на основе созданных набора данных и таблицы
    *     я авторизован как "Гарри"
    *     я на странице карты проекта, спозиционированной на объектах созданного слоя: "4"
    *     в форме просмотра объекта, я перехожу на вкладку просмотра геометрии
    *     у координаты с номером 1 в поле `X` устанавливаю значение 4999900
    When  я нажимаю на стрелку назад в панели просмотра объекта
    *     в появившемся диалоговом окне подтверждения нажимаю на кнопку "Всё равно закрыть"
    *     в списке слоёв в меню слоя "просто слой" я выбираю пункт "Перейти к слою"
    Then  на карте отображается "фича в исходном состоянии"
