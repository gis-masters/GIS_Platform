Feature: Форма объекта
  ...
  Scenario: Вкладка просмотра геометрии, в режиме чтения корректно отображает геометрию
    Given существует заготовленная схема "Все типы данных только для чтения"
    *     существует набор данных, созданный пользователем "Гарри"
    *     внутри созданного набора данных существует таблица по схеме "Все типы данных только для чтения" созданная пользователем "Гарри"
