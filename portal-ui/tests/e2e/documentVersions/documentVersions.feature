Feature: Версии документа
  ...
  Background:
    ...
    *     в библиотеке документов "документы с простым контент типом" существует документ с контент типом "doc_v1" c единственным полем `Название` равным "Тест док", доступный пользователю "Гарри"
  ...
  Scenario Outline: В библиотеке без версионирования у документа недоступна кнопка "Версии документа"
    ...
    *     в библиотеке документов "документы с простым контент типом без версионирования" существует документ с контент типом "doc_v1" c единственным полем `Название` равным "Тест док", доступный пользователю "Гарри"
