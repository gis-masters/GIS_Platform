Feature: Управление данными: Управление проектами
  ...
  #  Нестабильно, видимо показывает разные цифры, локально работает с [3495323,5357208,4169915,5896340]
  #  починить в 3896
  @skip()
  Scenario: В форме создания нового проекта поле Bbox заполнено значением по умолчанию
    Given я авторизован как "Гарри"
    *     я на странице `Проекты` в управлении данными
    When  я открываю форму создания нового проекта
    Then  в диалоговом окне формы текстовое поле "Область показа*" имеет значение "[3510207,5370754,4155031,5882794]"
