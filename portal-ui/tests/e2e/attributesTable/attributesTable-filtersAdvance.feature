Feature: Атрибутивная таблица: фильтрация
  ...
  Scenario: Фильтр остаётся активным при сворачивании и разворачивании атрибутивной таблицы
    ...
    *     пользователем "Гарри" внутри созданного набора данных создана таблица "для фильтрации" по схеме "Схема для тестирования сортировки"
    *     пользователем "Гарри" создан проект "для фильтрации"
    *     в созданном проекте создан слой "для фильтрации" на основе созданных набора данных и таблицы
    *     таблица наполнена данными "данные для тестирования сортировки", созданными "Гарри"
    *     я авторизован как "Гарри"
    *     я нахожусь на странице карты проекта
    *     открыта атрибутивная таблица созданного слоя
    When  в атрибутивной таблице я ввожу в поле "Поле STRING" значение "world"
  ...
  Scenario: Фильтрация НЕ работает для полей следующих типов: GEOMETRY, URL, FIAS, FILE
    Given существует заготовленная схема "Схема содержащая все типы данных"
    *     пользователем "Гарри" внутри созданного набора данных создана таблица "для фильтрации" по схеме "Схема содержащая все типы данных"
    *     пользователем "Гарри" создан проект "для фильтрации"
    *     в созданном проекте создан слой "для фильтрации" на основе созданных набора данных и таблицы
    *     таблица наполнена данными "данные для тестирования сортировки", созданными "Гарри"
    *     я авторизован как "Гарри"
  ...
  Scenario: Пользователь может отфильтровать данные по выделению оставляя только выделенные
    ...
    *     пользователем "Гарри" создан проект "для фильтрации"
    *     в созданном проекте создан слой "для фильтрации" на основе созданных набора данных и таблицы
    *     таблица наполнена данными "тестирование фильтрации", созданными "Гарри"
    *     я авторизован как "Гарри"
    ...
    *     открыта атрибутивная таблица созданного слоя
    ...
    Then  результат фильтрации в атрибутивной таблице по полю "ID" соответствует ожидаемому "1, 3"
  ...
  Scenario: Пользователь может отфильтровать данные по выделению оставляя только НЕ выделенные
    ...
    *     открыта атрибутивная таблица созданного слоя
    ...
    Then  результат фильтрации в атрибутивной таблице по полю "ID" соответствует ожидаемому "2"

  Scenario Outline: Пользователь может отфильтровать данные по двум фильтрам (по выделению и по полю STRING)
  В данном сценарии важна последовательность использования фильтра. Есть смежный сценарий, с обратной последовательностью: (по полю STRING и по выделению)
    ...
    *     открыта атрибутивная таблица созданного слоя
    When  в атрибутивной таблице я фильтрую по выделению оставляя только выделенные
    And   в атрибутивной таблице я ввожу в поле "<Фильтр по колонке:>" значение "<Значение фильтра:>"
    Then  результат фильтрации в атрибутивной таблице по полю "<Проверяемая колонка:>" соответствует ожидаемому "<Ожидаемый результат:>"

    Examples:
      | Выбраны объекты с id: | Фильтр по колонке: | Значение фильтра: | Проверяемая колонка: | Ожидаемый результат: |
      | 1, 2                  | Поле STRING        | s                 | ID                   | 2                    |

  Scenario Outline: Пользователь может отфильтровать данные по двум фильтрам (по полю STRING и по выделению)
  В данном сценарии важна последовательность использования фильтра. Есть смежный сценарий, с обратной последовательностью: (по выделению и по полю STRING)
    ...
    *     я нахожусь на странице карты проекта, спозиционированной на объектах созданного слоя
      ...
      | 1, 2                     |
    *     открыта атрибутивная таблица созданного слоя
    When  в атрибутивной таблице я ввожу в поле "<Фильтр по колонке:>" значение "<Значение фильтра:>"
  ...
  Scenario Outline: Пользователь может отфильтровать данные по выделению с большим количеством объектов 500+
    Given существует заготовленная схема "Схема содержащая все типы данных"
    *     пользователем "Гарри" внутри созданного набора данных создана таблица "для фильтрации" по схеме "Схема содержащая все типы данных"
    *     пользователем "Гарри" создан проект "для фильтрации"
    *     в созданном проекте создан слой "для фильтрации" на основе созданных набора данных и таблицы
    *     таблица наполнена данными "данные в количестве 628 для тестирования фильтрации", созданными "Гарри"
    *     я авторизован как "Гарри"
    *     я нахожусь на странице карты проекта, спозиционированной на объектах созданного слоя
      | Идентификаторы объектов: |
      | 1, 2                     |
    *     открыта атрибутивная таблица созданного слоя
    ...
    Examples:
      | Выбраны объекты с id: | Проверяемая колонка: | Ожидаемый результат: |
      | 1, 2                  | ID                   | 1, 2                 |
