...
const propertyAutoGeneratedArea: PropertySchema = {
  name: 'area_generated',
...
const propertyAutoGeneratedLength: PropertySchema = {
...
export const testSchemas: { [key: string]: Schema } = {
  ...
  'Схема с калькулируемым полем': getSchemaWithCalculatedArea(),
  'тестирование копирования 1': testSchemaWithAllTypes,
  'тестирование копирования 2': getSchemaForCopyTestVariant2()
};
...
function getSchemaWithCalculatedArea(): Schema {
  const copy: Schema = cloneDeep(testSchemaWithAllTypes);

  copy.name = `${testSchemaWithAllTypes.name}_SwCA`;
  copy.properties.push(propertyAutoGeneratedArea, propertyAutoGeneratedLength);

  return copy;
}

function getSchemaForCopyTestVariant2(): Schema {
  const copy: Schema = cloneDeep(testSchemaWithAllTypes);

  copy.name = `${testSchemaWithAllTypes.name}_SfCT-V2`;

  const fieldChoice = copy.properties.find(property => property.name === 'field_choice');
  if (fieldChoice) {
    const index = copy.properties.indexOf(fieldChoice);
