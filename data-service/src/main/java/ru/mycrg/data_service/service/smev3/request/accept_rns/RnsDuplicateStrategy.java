...
public class RnsDuplicateStrategy implements IRnsRequestDocumentCreator {
    ...
    private static XWPFDocument createTemplate(RequestType request) {
        ...
        if (dataProvider.getOrgFullName(request).equals("")) {
            addText(document, "от кого: ____________________________________", ParagraphAlignment.LEFT, 3875, 5, 12);
        } else {
            addTextWithUnderlineText(document, "от кого:", "      " + dataProvider.getOrgFullName(request) + "    ",
        ...
        if (dataProvider.getOrgPhone(request).equals("")) {
            addText(document, "____________________________________________", ParagraphAlignment.LEFT, 3875, 5, 12);
        } else {
        ...
        addTextWithSpacingAndUnderline(document,
                                       ...
                                       150, 11, "№ " + dataProvider.getPermitNumber(request));
        addTextWithSpacingAndUnderlineAndTextAfter(document, "выданного ", 5, 11,
