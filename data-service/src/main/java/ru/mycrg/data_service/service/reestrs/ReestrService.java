...
public class ReestrService {
    ...
    public Page<RecordDto> getAll(String tableName, Pageable pageable, String ecqlFilter) {
        ResourceQualifier reestrQualifier = new ResourceQualifier(SYSTEM_SCHEMA_NAME, tableName);

        SchemaDto schema = getSchema(tableName);

        List<RecordDto> result = recordsDao.findAll(reestrQualifier, ecqlFilter, schema, pageable).stream()
                                           .map(RecordDto::new)
