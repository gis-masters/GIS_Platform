...
public class DocumentSearchEngine implements IFullTextSearchEngine {
    ...
    public Page<FtsResponseDto> search(FtsRequest request) {
        ...
        List<FtsItem> founded = ftsDao.search(DOCUMENTS, new ArrayList<>(), dto.getEcqlFilter(), dto.getText(),
                                              getBound(dto), request.getPageable());
        Long total = ftsDao.countTotal(DOCUMENTS, new ArrayList<>(), dto.getEcqlFilter(), dto.getText(), getBound(dto));
    ...
    private float getBound(FtsRequestDto dto) {
