...
public class UpdateTableRequestHandler implements IRequestHandler<UpdateTableRequest, Voidy> {
    ...
    public UpdateTableRequestHandler(IAuthenticationFacade authenticationFacade,
    ...
    public Voidy handle(UpdateTableRequest request) {
        ...
        String tableId = tQualifier.getTable();

        SchemasAndTables tableForUpdate;
        tableForUpdate = schemasAndTablesRepository
                .findByIdentifier(tableId)
                .orElseThrow(() -> new NotFoundException("Not found table: " + tableId));

        if (isOwner(tQualifier)) {
            ...
            tableForUpdate.setLastModified(LocalDateTime.now());

            request.setTableModel(tableForUpdate);

            schemasAndTablesRepository.save(tableForUpdate);
        } else {
            throw new ForbiddenException("Недостаточно прав для редактирования информации о слое: " + tableId);
        }
