...
@Component
public class GpzuFixErrorStrategy implements IGpzuRequestDocumentCreator {
    ...
    private static XWPFDocument createTemplate(RequestType request) {
        ...
        addCenterText(document, "об исправлении допущенных опечаток и ошибок в градостроительном плане", 13, false);
        addCenterText(document, "земельного участка", 13, false);
        addText(document, "");
        ...
        addCenterText(document, "1. Сведения о заявителе<1>", 13, false);
        ...
        addText(document, "");
        addCenterText(document, "2. Сведения о выданном градостроительном плане земельного участка, содержащем", 13, false);
        addCenterText(document, "опечатку/ошибку", 13, false);

        XWPFTable table2 = document.createTable(2, 4);
        setColumnWidth(table2, 0, 1000);
        setColumnWidth(table2, 1, 7000);
        setColumnWidth(table2, 2, 4000);
        setColumnWidth(table2, 3, 1000);
        setTableNode(table2, 0, "N", "Орган, выдавший градостроительный план земельного участка", "Номер документа",
                     "Дата документа");
        setTableNode(table2, 1, " ", dataProvider.getGPZUInformationIssuer(), dataProvider.getGPZUInformationNumber(),
                     dataProvider.getGPZUInformationDate());
        addText(document, "");

        addCenterText(document, "3. Обоснование для внесения исправлений в градостроительный план земельного", 13, false);
        addCenterText(document, "участка", 13, false);


        XWPFTable table3 = document.createTable(2, 4);
        setColumnWidth(table3, 0, 1000);
        setColumnWidth(table3, 1, 7000);
        setColumnWidth(table3, 2, 4000);
        setColumnWidth(table3, 3, 1000);
        setTableNode(table3, 0, "N", "Данные (сведения), указанные в градостроительном плане земельного участка", "Данные " +
                             "(сведения), которые необходимо указать в градостроительном плане земельного участка",
                     "Обоснование с указанием реквизита(ов) документа(ов), документации, на основании которых принималось решение о выдаче градостроительного плана земельного участка");
        setTableNode(table3, 1, " ", dataProvider.getCurrentVersionData(), dataProvider.getNewVersionData(),
                     " ");
        addText(document, "");

        addText(document, "     Прошу внести исправления в градостроительный план земельного участка, содержащий " +
                "опечатку/ошибку.");
        ...
        addTextWithUnderline(document, dataProvider.getPhoneNumber() + " " + dataProvider.getEmail(), ParagraphAlignment.LEFT,
                             0, 5, 12);
        addText(document, "Результат рассмотрения настоящего заявления прошу:");
        XWPFTable table4 = document.createTable(4, 2);
        setColumnWidth(table4, 0, 11500);
        setColumnWidth(table4, 1, 1500);
        setTableNode(table4, 0, "направить в форме электронного документа в личный кабинет в федеральной " +
        ...
        setTableNode(table4, 1, "выдать на бумажном носителе при личном обращении в уполномоченный орган " +
        ...
        setTableNode(table4, 2, "направить на бумажном носителе на почтовый адрес: ",
                     getCheckmarkSymbol(dataProvider, 2));
        mergeCellsAndSetValue(table4, 3, "Указывается один из перечисленных способов", false, ParagraphAlignment.CENTER);
        addText(document, "");
        ...
        addText(document, "");
        addText(document, "     --------------------------------");
        addText(document, "     <1> Заявителями являются правообладатели земельных участков, а также иные лица,\n" +
                "указанные в части 1.1 статьи 57.3 Градостроительного кодекса Российской Федерации.");
