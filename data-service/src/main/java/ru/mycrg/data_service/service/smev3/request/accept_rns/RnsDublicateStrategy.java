...
@Component
public class RnsDublicateStrategy implements IRnsRequestDocumentCreator {
    ...
    private static XWPFDocument createTemplate(RequestType request) {
        ...
        String issueDate = oRecipientPersonalData.map(RecipientPersonalDataType::getDocumentPersonal)
                                                 ...
                                                 .orElse(oDelegatePersonalData
                                                                 ...
                                                                 .map(DocumentPersonalType::getIssuedate)
                                                                 .orElse(""));
        String issueOrg = oRecipientPersonalData.map(RecipientPersonalDataType::getDocumentPersonal)
                                                .map(DocumentPersonalType::getIssueorg)
                                                .orElse(oDelegatePersonalData
                                                                .map(DelegatePersonalDataType::getDocumentPersonal)
                                                                .map(DocumentPersonalType::getIssueorg)
                                                                .orElse(""));
        String nameDoc = oRecipientPersonalData.map(RecipientPersonalDataType::getDocumentPersonal)
        ...
        addTextWithUnderline(document,
                             "____" + issueOrg + "__" + nameDoc + "__" + regAddress + "/" + factAddress + "____",
        ...
        addTextWithSpacingAndUnderline(document,
                                       ...
                                       5, 11, "№ " + permitNumber);
        addTextWithSpacingAndUnderline(document, "выданного ", 5, 11, permitDate);
