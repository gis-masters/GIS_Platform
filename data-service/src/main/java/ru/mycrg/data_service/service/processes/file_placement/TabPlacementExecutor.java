...
public class TabPlacementExecutor implements IExecutor<ImportReport>, IFilePlacer {
    ...
    private static final Logger log = LoggerFactory.getLogger(TabPlacementExecutor.class);
    ...
    public TabPlacementExecutor(FileRepository fileRepository,
    ...
    /**
     * TAB файл это "составной" файл.
     * <p>
     * Для публикации слоя на геосервере, на его основе, требуется наличие одинаково именованных 4-х файлов: some.tab,
     * some.map, some.id, some.dat.
     */
    @Override
    @Transactional
    public ImportReport execute() {
        ...
        Set<String> requiredFiles = Set.of(baseTitle + ".tab",
                                           baseTitle + ".id",
                                           baseTitle + ".map",
                                           baseTitle + ".dat");
        ...
        if (countSameFiles(foundFiles) != requiredFiles.size()) {
            ...
            if (foundFiles.size() != requiredFiles.size()) {
                ...
                throw new IllegalStateException("Для размещения TAB файла не найдены все требуемые файлы");
            }
