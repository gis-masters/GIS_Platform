...
public class CopyTableRecordsRequestHandler implements IRequestHandler<CopyTableRecordsRequest, Voidy> {
    ...
    public Voidy handle(CopyTableRecordsRequest request) {
        ...
        List<String> sourceColumnNames = ddlTablesSpecial.getAllColumnNames(sourceQualifier.getTable());
        List<String> targetColumnNames = ddlTablesSpecial.getAllColumnNames(targetQualifier.getTable());

        checkIsTableColumnsExist(schemaSource, schemaTarget, sourceColumnNames, targetColumnNames);
        ...
        List<Long> copiedIds = spatialRecordsDao.copyRecords(sourceQualifier.getTableQualifier(),
                                                             ...
                                                             featureIds, systemAutogeneratedFieldForTargetTable);
    ...
    private void checkIsTableColumnsExist(SchemaDto schemaSource, SchemaDto schemaTarget,
                                          List<String> sourceColumnNames, List<String> targetColumnNames) {
        Map<String, Object> systemFields = generateSystemFields();

        Set<String> propNamesSource = schemaSource.getProperties()
                                                  .stream()
                                                  .map(SimplePropertyDto::getName)
                                                  .filter(name -> !systemFields.containsKey(name))
        ...
        throwIfNotMatchTableColumns(propNamesSource, sourceColumnNames);
        throwIfNotMatchTableColumns(propNamesTarget, targetColumnNames);
    }
