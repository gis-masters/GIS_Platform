package ru.mycrg.data_service.dao;

import org.jetbrains.annotations.NotNull;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.jdbc.core.JdbcTemplate;
import org.springframework.stereotype.Service;
import ru.mycrg.data_service.dao.exceptions.CrgDaoException;
import ru.mycrg.data_service_contract.enums.GeometryType;

import static ru.mycrg.data_service.dao.DaoProperties.DEFAULT_GEOMETRY_COLUMN_NAME;

@Service
public class GeometryHelper {

    private final Logger log = LoggerFactory.getLogger(GeometryHelper.class);

    private final JdbcTemplate jdbcTemplate;

    public GeometryHelper(JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
    }

    public Integer countInvalid(String schema, String table) throws CrgDaoException {
        try {
            String sql = String.format("SELECT COUNT(*) FROM %s.%s WHERE public.st_isvalid(%s)=false",
                                       schema, table, DEFAULT_GEOMETRY_COLUMN_NAME);

            return jdbcTemplate.queryForObject(sql, Integer.class);
        } catch (Exception e) {
            String msg = "Не удалось выполнить подсчет невалидных объектов для: " + schema + "." + table;

            throw new CrgDaoException(msg, e.getCause());
        }
    }

    public void makeValid(String schema, String table) throws CrgDaoException {
        try {
            String geometryColumnName = DEFAULT_GEOMETRY_COLUMN_NAME;
            String sql = String.format("UPDATE %s.%s SET %s=public.st_makevalid(%s) WHERE public.st_isvalid(%s)=false",
                                       schema, table, geometryColumnName, geometryColumnName, geometryColumnName);

            jdbcTemplate.execute(sql);
        } catch (Exception e) {
            String msg = "Ошибка при исправлении геометрии для: " + schema + "." + table;

            throw new CrgDaoException(msg, e.getCause());
        }
    }

    public void convertGeometryCollectionTo(String schema,
                                            String table,
                                            @NotNull GeometryType geometryType) throws CrgDaoException {
        try {
            String geometryColumnName = DEFAULT_GEOMETRY_COLUMN_NAME;
            String sql = String.format("UPDATE %s.%s SET %s=public.st_CollectionExtract(%s, %d)",
                                       schema, table, geometryColumnName, geometryColumnName,
                                       geometryTypeAsNumber(geometryType));

            log.debug("convertGeometryCollectionTo sql: {}", sql);

            jdbcTemplate.execute(sql);
        } catch (Exception e) {
            String msg = "Ошибка трансформации коллекции. Тип геометрии: " + geometryType;

            throw new CrgDaoException(msg, e.getCause());
        }
    }

    /**
     * https://postgis.net/docs/ST_CollectionExtract.html
     */
    private int geometryTypeAsNumber(GeometryType geometryType) throws CrgDaoException {
        switch (geometryType) {
            case POINT:
                return 1;
            case MULTI_LINE_STRING:
                return 2;
            case MULTI_POLYGON:
                return 3;
            default:
                throw new CrgDaoException("Not supported geometry type: " + geometryType);
        }
    }
}
