...
public class SqlBuilder {
    ...
    public static String buildHeadlinesQuery(String baseWord, Set<String> words) {
    ...
    //Уйти от двух листов пропсов. Какой смысл если это 1 и тот же лист
    @NotNull
    public static String buildCopyGpkgFileQuery(ResourceQualifier sourceTable,
                                                ResourceQualifier targetTable,
                                                List<SimplePropertyDto> sourcePropsWithoutSystemFields,
                                                List<SimplePropertyDto> targetProps,
                                                Map<String, Object> systemAutogeneratedFieldForTargetTable) {
        Optional<String> geometrySourcePropertyName = getPropertyNameByType(ValueType.GEOMETRY,
        ...
        geometryTargetPropertyName.ifPresent(s -> updateGeometryPropertyName(sourcePropsWithoutSystemFields, s));

        String insertTo = "INSERT INTO " + targetTable.getQualifier();
        String data = mappingColumnsForGpkg(sourcePropsWithoutSystemFields, targetProps,
                                            systemAutogeneratedFieldForTargetTable);
    ...
    public static String generatePropertySqlString(@NotNull SimplePropertyDto attrDescription) {
        ...
        switch (attrDescription.getValueTypeAsEnum()) {
            ...
            case CHOICE:
                ...
                break;
            case STRING:
                result = attrDescription.getMaxLength() == null
                        ? attrDescription.getName() + " VARCHAR"
                        : attrDescription.getName() + " VARCHAR(" + attrDescription.getMaxLength() + ")";
                break;
            case DOUBLE:
    ...
    private static String mappingColumnsForGpkg(List<SimplePropertyDto> sourcePropsWithoutSystemFields,
                                                List<SimplePropertyDto> targetProps,
                                                Map<String, Object> systemAutogeneratedFieldForTargetTable) {
        String pre = " (";
        String post = ") ";
        ...
        for (SimplePropertyDto sourceProperty: sourcePropsWithoutSystemFields) {
            ...
            List<SimplePropertyDto> filteredProps = targetProps
                    ...
                    .filter(equalByName(sourceName))
                    .filter(notGeneratedProperty())
                    .filter(compatibleByTypeForGpkg(sourceValueType))
                    .collect(Collectors.toList());

            if (!filteredProps.isEmpty()) {
                SimplePropertyDto targetProperty = filteredProps.get(0);
                ValueType targetValueType = targetProperty.getValueTypeAsEnum();

                targetColumns.append(sourceName).append(", ");

                // Специальная обработка для UUID полей
                if (targetValueType == ValueType.UUID) {
                    sourceColumns.append("\"").append(sourceName).append("\"::uuid, ");
                }
                // Специальная обработка для BOOLEAN полей
                else if (targetValueType == ValueType.BOOLEAN) {
                    sourceColumns.append("CASE WHEN \"").append(sourceName).append("\" IS NULL THEN NULL ")
                                 .append("WHEN \"").append(sourceName).append("\"::integer = 0 THEN false ")
                                 .append("ELSE true END, ");
                }
                // Специальная обработка для DATETIME полей - преобразование из with time zone в without time zone
                else if (targetValueType == ValueType.DATETIME) {
                    sourceColumns.append("(\"").append(sourceName).append("\" AT TIME ZONE 'UTC'), ");
                }
    ...
    @NotNull
    private static Predicate<SimplePropertyDto> compatibleByTypeForGpkg(ValueType sourceValueType) {
        return prop -> {
            ValueType targetType = prop.getValueTypeAsEnum();

            // STRING может быть преобразован в любой тип
            if (targetType.equals(ValueType.STRING)) {
                return true;
            }

            // STRING может быть преобразован в CHOICE
            if (sourceValueType.equals(ValueType.STRING) && targetType.equals(ValueType.CHOICE)) {
                return true;
            }

            // Для GPKG: STRING может быть преобразован в UUID
            if (sourceValueType.equals(ValueType.STRING) && targetType.equals(ValueType.UUID)) {
                return true;
            }

            // Для GPKG: INTEGER может быть преобразован в BOOLEAN
            if (sourceValueType.equals(ValueType.INT) && targetType.equals(ValueType.BOOLEAN)) {
                return true;
            }

            // Обычная проверка совместимости типов
