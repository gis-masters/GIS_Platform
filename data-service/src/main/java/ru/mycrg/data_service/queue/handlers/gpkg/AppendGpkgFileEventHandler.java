...
public class AppendGpkgFileEventHandler implements IEventHandler {
    ...
    public void handle(IMessageBusEvent mqEvent) {
        ...
        try {
            findAndAppendFiles(pathToFile, event.getDbName(), event.getResourceProjections());

            log.debug("Успешно добавили доп. медиа в файл: ({})", pathToFile);

            messageBus.produce(new AppendGpkgFileBackwardEvent(event.getBusinessKey(), DONE));
        } catch (Exception e) {
    ...
    public void findAndAppendFiles(String pathToGpkg, String dbName, List<ExportResourceModel> resources) {
