...
@Service
public class MidPlacedSucceededEventHandler implements IEventHandler {

    private final Logger log = LoggerFactory.getLogger(MidPlacedSucceededEventHandler.class);

    private final ProcessService processService;
    private final WsNotificationService wsNotificationService;

    public MidPlacedSucceededEventHandler(ProcessService processService,
    ...
    public String getEventType() {
        return MidPlacedSucceededEvent.class.getSimpleName();
    }

    @Override
    public void handle(IMessageBusEvent mqEvent) {
        try {
            MidPlacedSucceededEvent event = (MidPlacedSucceededEvent) mqEvent;
            ...
            log.error("Не удалось корректно обработать MidPlacedSucceededEventHandler. Причина: {}", e.getMessage());
        }
    }
}
