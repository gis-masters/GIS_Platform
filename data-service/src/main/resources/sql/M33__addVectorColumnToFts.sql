-- Добавим новые колонки в основные таблицы
ALTER TABLE data.fts_documents
    ADD COLUMN IF NOT EXISTS vector_data tsvector GENERATED ALWAYS AS (to_tsvector('russian', concatenated_data)) STORED;
...
ALTER TABLE IF EXISTS data.fts_dictionary
    OWNER to fiz;
...
CREATE INDEX IF NOT EXISTS trgm_gist_indx
    ON data.fts_dictionary USING gist (word public.gist_trgm_ops);
