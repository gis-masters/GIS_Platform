package ru.mycrg.integration_service.bpmn.publication.mid.gis_layer;
...
@Service("gisCreateMidLayerDelegate")
public class GisCreateMidLayerDelegate implements JavaDelegate {

    private final Logger log = LoggerFactory.getLogger(GisCreateMidLayerDelegate.class);

    private final BaseHttpService baseHttpService;

    public GisCreateMidLayerDelegate(BaseHttpService baseHttpService) {
        this.baseHttpService = baseHttpService;
    }

    @Override
    public void execute(DelegateExecution execution) throws Exception {
        log.debug("execute GisCreateMidLayerDelegate");
        ...
        try {
            ...
            if (response.isSuccessful()) {
                log.debug("Successfully created MID layer with params: [{}]", dto);
            ...
            log.error(format("Failed to execute step: 'gisCreateMidLayerDelegate'. Cause: %s", e.getMessage()), e);
    ...
    private String baseFailMsg() {
        return "Не удалось создать MID слой на gis сервисе";
    }
}
